 ...
 * <p>
 * 示例使用代码：
 * AutoCheck.getInstance(getApplicationContext()).check(ttsConfig, new Handler() {
 *
...
public class AutoCheck {
    ...
    public void check(final TTSConfig TTSConfig, final Handler handler) {
        Thread t = new Thread(new Runnable() {
            @Override
            public void run() {
                AutoCheck obj = innerCheck(TTSConfig);
                isFinished = true;
    ...
    private AutoCheck innerCheck(TTSConfig config) {
