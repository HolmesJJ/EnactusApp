...
public class GazeService extends Service implements ViewTreeObserver.OnGlobalLayoutListener, GazeListener {
    ...
    private static final String TAG = GazeService.class.getSimpleName();
        ...
        if (!IS_USE_GAZE_FILER) {
            if (isGazeMode) {
                synchronized (mGazeCoordEvent) {
                    int gazeState = mGazeCoordEvent.getGazePoint().getState();
                    if (gazeState == TrackingState.TRACKING) {
                        mainLooper.post(new Runnable() {
                            @Override
                            public void run() {
                                moveCrossLines(x, y);
                        ...
                        mainLooper.post(new Runnable() {
                            @Override
                            public void run() {
                                pbGaze.setProgress(0);
                            }
                        });
                    }
                }
            } else {
                EventBus.getDefault().post(new GazeCoordEvent(new GazePoint(x, y, state)));
            }
        }
    }
    ...
    public void onFilteredGazeCoord(long timestamp, float x, float y, int state) {
        if (IS_USE_GAZE_FILER) {
            if (isGazeMode) {
                synchronized (mGazeCoordEvent) {
                    mGazeCoordEvent.setGazePoint(new GazePoint(x, y, state));
                    int gazeState = mGazeCoordEvent.getGazePoint().getState();
                    if (gazeState == TrackingState.TRACKING) {
                        mainLooper.post(new Runnable() {
                            @Override
    ...
    private void moveCrossLines(float x, float y) {
        // 同时移动
        if (!isMoved) {
            isMoved = true;
    ...
    public void onGazeEyeMovement(long timestamp, long duration, float x, float y, int state) {
        if (isGazeMode) {
            synchronized (mGazeEyeMovementEvent) {
                mGazeEyeMovementEvent.setGazePoint(new GazePoint(x, y, state));
                countFixation(mGazeEyeMovementEvent);
            }
        } else {
            EventBus.getDefault().post(new GazeEyeMovementEvent(new GazePoint(x, y, state)));
        }
    }
            ...
            if (preEyeMovementState == EyeMovementState.FIXATION) {
                ...
                if (fixationCounter <= 25) {
                    mainLooper.post(new Runnable() {
                        @Override
                        public void run() {
                            pbGaze.setProgress(fixationCounter * 4);
                        }
                    });
                    if (fixationCounter == 25) {
                        // 模拟点击
